...
const UpdateRequestUser = ({ item, text }) => {
  ...
  const handleOptionChange = (name) => {
    ...
    if (lowercaseName === "role") {
      ...
      setInputData({ ...inputdata, role: "", username: lowercaseName });
    } else {
      setInputData({
        ...inputdata,
        role: name,
  ...
  const handleSubmit = async (event) => {
    ...
    setUsenameInvalid(false);
      ...
      if (!findExistingUserName) {
        try {
          let image = inputdata.image; // Validate username format
          if (selectedFile) {
...
export default UpdateRequestUser;
